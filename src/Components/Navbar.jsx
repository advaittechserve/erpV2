...
const Navbar = () => {
  ...
  const fetchData = async (userId) => {
    try {
        ...
        const admindetailsData = response.data[0].id;
...
const handleEditUser = async () => {
    try {
        const adminData = await fetchData(userId);
        navigate(`/EditUser/${adminData}`, { state: { adminData } });
    } catch (error) {
...
const handleChangePassword = () => {
  ...
  return (
    <>
              ...
              >
                ...
                >
                  <span className="initials">
                    {userId.charAt(0) + "  " + userId.charAt(1)}
                  </span>
                </div>
              </button>
              {userMenuOpen && (
      ...
      {/* Pass sidebarOpen state and toggleSidebar function as props */}
    </>
  );
};
