...
const Sidebar = ({ isOpen, toggleSidebar }) => {
  ...
  return (
    <aside
      className={`fixed top-0 left-0 z-40 w-50 min-h-screen pt-20 transition-transform ${isOpen ? "translate-x-0" : "-translate-x-full"
        } bg-gray-800 border-r-4 xl:translate-x-0 dark:bg-yellow-800 dark:border-yellow-700 mt-5 p-1`}
      id="logo-sidebar"
    >
            ...
            <li key={index}>
              {menuItem.children ? (
                <div className="relative">
                  <button
                    type="button"
                    className={`sidebar-list flex items-center justify-between text-white rounded-xl dark:text-white hover:text-black dark:hover:bg-gray-700 group focus:outline-none ${activeMenuItem === menuItem.link
                  ...
                  >
                    ...
                    <svg
                      className={`w-5 h-8 ${isFormMenuOpen ? "transform rotate-180" : ""
                        }`}
                      aria-hidden="true"
                      xmlns="http://www.w3.org/2000/svg"
                      viewBox="0 0 20 20"
                    ...
                    <ul className={`py-2 space-y-2 my-1"${isFormMenuOpen ? "block" : "hidden"}`}>
                      {menuItem.children.map((childItem, childIndex) => (
                        <li key={childIndex}>
                          <button
                            type="button"
                            className={`sidebar-list text-white rounded-xl dark:text-white hover:text-black ${activeMenuItem === childItem.link ? "active" : ""
                              }`}
                ...
                <button
                  type="button"
                  className={`sidebar-list flex items-center text-white rounded-xl dark:text-white hover:text-black dark:hover:bg-gray-700 group ${activeMenuItem === menuItem.link ? "active" : ""
                    }`}
                  onClick={() => handleMenuItemClick(menuItem.link)}
                >
                  <svg
                    className={`svgcss flex-shrink-0 w-5 h-8 text-white transition duration-75 dark:text-gray-400 group-hover:text-gray-900 dark:group-hover:text-white ${activeMenuItem === menuItem.link ? "active" : ""
                      }`}
                    aria-hidden="true"
                    xmlns="http://www.w3.org/2000/svg"
                    fill="currentColor"
