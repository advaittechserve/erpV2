  ...
  const fetchAtmIds = async (customerId) => {
    try {
      const response = await fetch(
        `http://localhost:5000/atm?CustomerId=${customerId}`
      );
      if (!response.ok) {
        ...
        for (const atmId of selectedAtmIds) {
          ...
          if (response.data && response.data.length > 0) {
            ...
            setConfirmAction(() => async () => {
              ...
              await axios.post(
                "http://localhost:5000/api/updateServicesData",
                updateData
              );
            ...
            setModalMessage(
              `Do you want to insert service details for ATM ID ${atmId}?`
            );
            setConfirmAction(() => async () => {
              await axios.post(
                "http://localhost:5000/api/insertServicesData",
                formData
              );
            });
          }
        ...
        <div className="grid gap-4 mb-6 md:grid-cols-3 mt-4">
          <Select
            ...
            value={options.filter((option) =>
              selectedAtmIds.includes(option.value)
            )}
            options={options}
